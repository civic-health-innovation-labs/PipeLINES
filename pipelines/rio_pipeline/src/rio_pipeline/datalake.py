...
from rio_pipeline.common_types import DatalakeZone, Table


def _table_schema_removal(full_table_name: str) -> str:
    """Removes schema from table, e.g. 'dbo.xyz' -> 'xyz'
    Args:
        full_table_name (str): Full table name (or short name).
    Returns:
        str: table name without prefix.
    """
    if "." in full_table_name:
        _start_idx: int = full_table_name.index(".") + 1
        return full_table_name[_start_idx:]
...
def construct_uri(spark: SparkSession, zone: DatalakeZone, table: str) -> str:
    ...
    table_without_db_schema = _table_schema_removal(table)
...
def create_table_in_unity_catalog(
    ...
    _tbl = _table_schema_removal(table_name)
    spark_session.sql(
