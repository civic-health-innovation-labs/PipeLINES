...
def _get_tables_and_columns_from_sql(
    ...
    reader = (
        ...
        .option("user", config.smart_username)
        .option("password", config.smart_password)
    )
    if config.authentication_method == AuthenticationMethod.APP_REGISTRATION:
        reader = reader.option("authentication", "ActiveDirectoryServicePrincipal")
...
def upsert_append_only_table(
    ...
    """
    # Drop duplicities on the key column (as RiO data are often defective).
