...
from rio_pipeline.stages.pseudonymisation.transform import pseudo_transform
...
from rio_pipeline.config import RUN_FREE_TEXT_ANONYMISATION
...
def test_transformer_remove_columns_multiple_columns(
    ...
    expected_df = spark_session.createDataFrame(
        [("This is a note value", 1), ("This is a note value", 2)],
        ["NoteText", "ID"],
    )
    assert set(expected_df.columns) == set(output_df.columns)
    assert expected_df.collect() == output_df.collect()
...
def test_pseudo_transform_pseudonymises_free_text_name_and_location(
    ...
    input_df = spark_session.createDataFrame(
        [("John Smith is in London", 1), ("Adam is in London", 2)],
        ["NoteText", "ID"],
    )
    table: Table = Table(
        name="table1",
...
def test_pseudo_transform_hashes_id(
    ...
    expected_df = spark_session.createDataFrame(
        [
            (
                "e59cb3f3ffba6255f0f32b278a76f8a44780fde36bb7a1b3428a394ff4c39596",
                "Text1",
            )
        ],
        ["ID_hashed", "Text"],
