...
from rio_pipeline.config import ROW_HASH_COLUMN_NAME
from rio_pipeline.common_types import Table, IngestionType, ColumnType
from rio_pipeline.common_exceptions import SchemaValidationException
...
def test_validate_sql_source_against_schema(
    ...
    try:
        ...
        assert (
            "SuperUnclassified"
            in analysed_exception["analysed_tables"]["dbo.AmsAppointment"][
                "unclassified_columns"
            ]
        )
        assert len(
            analysed_exception["analysed_tables"]["dbo.AmsAppointment"][
                "classified_but_unexisting_columns"
            ]
        ) == (
            len(
                # Find analysed columns of dbo.AmsAppointment table
                [
                    tc.analysed_columns
                    for tc in TABLE_CONFIG
                    if tc.name == "dbo.AmsAppointment"
                ][0]
            )
            - 3
        )
    ...
    with engine.connect() as conn:
        conn.execute(text("DROP TABLE AmsAppointment;"))
...
def test_load_sql_table(
    ...
    with engine.connect() as conn:
        conn.execute(
            text(
                "CREATE TABLE PrgProgressNote ([SequenceID] int NOT NULL, [ClientID]"
        ...
        conn.execute(
            text(
                "INSERT INTO PrgProgressNote VALUES (1,'123','2016-05-03 15:20:00','The"
    ...
    df = load_sql_table(spark_session, config, "PrgProgressNote")
    ...
    # Cleaning work in the testing SQL instance (for other tests)
    with engine.connect() as conn:
        conn.execute(text("DROP TABLE PrgProgressNote;"))
        conn.commit()
